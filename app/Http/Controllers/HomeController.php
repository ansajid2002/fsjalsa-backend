...
class HomeController extends Controller
{
    ...
    public function vv_web_location($pincode)
    {
      $distance = DB::select('select * from users where pincode='.$pincode.'');
      $making_world = count($distance);
      if($making_world > 0){
        $sr = 1;
      }else{
        $sr = 0;
      ...
      if($making_world > 0){
         Session::put('pincode_of_session', $pincode);
      }else{
         Session::put('pincode_of_session', 0);
      }
      $result[] = ['attrn'=>$sr];
    ...
    public function createServicePerson(Request $request){
      return view('frontend.deliveryboy.create');
    }

    public function loginServicePerson(Request $request){
      return view('frontend.deliveryboy.login');
    }

    public function createServicePersonIn(Request $request){
      return $request->all();
    }

    public function sendSmsForRegOtpDel(){

    }

    function sendSmsForAll($otp, $mobileno){

      // GROUPS OF PARAMETER START HERE
      $baseUrl = "http://103.225.76.43/sms/user/urlsms.php";
      $userName = "username=benet";
      $password = "pass=Admin123@";
      $senderId = "senderid=ATGHAR";
      $receiver = "dest_mobileno=".$mobileno;
      $msg = "message=".str_replace(" ","%20","Hi, Your One Time Password(OTP) is ".$otp."%0aBy SABGHARPE.COM");
      //"message=".str_replace(" ","%20","Hi, Your One Time Password(OTP) is ".$otp.".\nBy SABGHARPE.COM\n");
      $res = "response=Y";
      // GROUPS OF PARAMETER ENDS HERE

      $curl = curl_init();
      $actualUrl = $baseUrl."?".$userName."&".$password."&".$senderId."&".$receiver."&".$msg."&".$res;
      curl_setopt_array($curl, array(
        CURLOPT_URL => $actualUrl,
    ...
    function sendSmsForDeliveryBoyRegister(Request $request){
      $checkUserAvailability = DB::table('users')->where('phone',$request->mobileNo)->get();
      if(count($checkUserAvailability) > 0){
        return 0;
      }else{
        $otp = rand(100000,999999);
        $mobileno = $request->mobileNo;
        $res = $this->sendSmsForAll($otp,$mobileno);
        if($res == 1){
          Session::put('DeliveryBoyLoginMobileNo',$request->mobileNo);
          Session::put('DeliveryBoyhasLoginOtp',$otp);
          return 1;
        }else{
          return 2;
        }
    ...
    function sendSmsForDeliveryBoyLogin(Request $request){
      $checkUserAvailability = DB::table('users')->where('phone',$request->mobileNo)->get();
      if(count($checkUserAvailability) > 0){
        $otp = rand(100000,999999);
        $mobileno = $request->mobileNo;
        $res = $this->sendSmsForAll($otp,$mobileno);
        if($res == 1){
          Session::put('ServiceBoyLoginMobileNo',$request->mobileNo);
          Session::put('ServiceBoyhasLoginOtp',$otp);
          return 1;
        }else{
          return 2;
        }
    ...
    function verifyDeliveryBoyOtpLogin(Request $request){
      if(Session::get('ServiceBoyhasLoginOtp') == $request->otpNo && Session::get('ServiceBoyLoginMobileNo') == $request->moboNo){
        $dst = DB::table('users')->where('phone',Session::get('ServiceBoyLoginMobileNo'))->first();
        $userdata = array(
          'phone' =>  $dst->phone,
          'password' => '12345678',
        );
        if (Auth::attempt($userdata))
          {
              Session::forget('ServiceBoyhasLoginOtp');
              Session::forget('ServiceBoyLoginMobileNo');
              return 1;
          }
          else
          {
    ...
    function serviceBoyLogout(){
      Auth::logout();
      return redirect('/login/service-person');
    }

    function verifyDeliveryBoyOtp(Request $request){
      if(Session::get('DeliveryBoyhasLoginOtp') == $request->otpNo && Session::get('DeliveryBoyLoginMobileNo') == $request->moboNo){
        Session::put('verifiedDeliveryBoyPhone',true);
        Session::put('Deliveryreadonly','readonly');
        return 1;
      }else{
        return 0;
      }
          ...
          $check = DB::table('users')->where('phone',$request->mobileno)->get();
          if(count($check) > 0){
            return redirect()->back()->with('error','phone no already registered');
          }
          $check = DB::table('users')->where('email',$request->email)->get();
          if(count($check) > 0){
            return redirect()->back()->with('error','email already registered');
          }
          $pic = $request->yourphoto;
          $path = 'public/register/deliveryboy/profile/';
          $filename = time() . 'profile.' . $pic->getClientOriginalExtension();
          $pic->move($path, $filename);
          $picker = $filename;

          $picAdhar = $request->aadhar_photo;
          $paths = 'public/register/deliveryboy/aadhar/';
          $filenames = time() . 'aadhar.' . $picAdhar->getClientOriginalExtension();
          $picAdhar->move($paths, $filenames);
          $picAdhar_name = $filenames;

        $rr =  DB::table('users')->insertGetId([
        ...
        $mr =  DB::table('staff')->insert([
          "user_id" => $rr,
          "role_id"=> 3
        ]);
        $dr = DB::table('delivery_boy_details')->insert([
          "user_id" => $rr,
          "house" => $request->house,
          ...
          "pic" => $picker,
          "aadhar_pic" => $picAdhar_name,
          "pan" => $request->pancardno,
          "dlc" => $request->dln,
          "bankname" => $request->bankname,
          "ifsc" => $request->ifsc,
          "bank_acc" => $request->bankaccno,
          "work_from" => $request->workfrom,
          "work_to" => $request->workto
        ]);
        if($rr){
          Session::forget('DeliveryBoyLoginMobileNo');
          Session::forget('Deliveryreadonly');
          Session::forget('DeliveryBoyhasLoginOtp');
          Session::forget('verifiedDeliveryBoyPhone');
          return redirect()->back()->with('success',"registered successfully. we will review your application and contact you.");
      ...
      public function deliveryOrderPending(){
        $deliveryOrder = DB::table('orders')->where('assign_to',auth()->user()->id)->get();
        return view('serviceBoy.pending',compact('deliveryOrder'));
      }
      public function deliveryOrderComplete(){
        $deliveryOrder = DB::table('orders')->where('assign_to',auth()->user()->id)->get();
        return view('serviceBoy.complete',compact('deliveryOrder'));
      }
      public function deliveryOrderPendingView($id){
        $order = DB::table('orders')->where('id',$id)->first();
        return view('serviceBoy.show',compact('order'));
      }
      public function cashCollectedDeliveryBoy($id){
        DB::table('orders')->where('id',$id)->update([
          'payment_status'=>"paid",
        ]);
        return redirect()->back()->with('success','Paid Successfully');
      }
      public function deliveryDoneDeliveryBoy($id){
        DB::table('order_details')->where('order_id',$id)->update([
          'delivery_status'=>"delivered",
        ]);
        return redirect('/deliveryboy/order/pending')->with('success','Delivery Done Successfully');
      }
