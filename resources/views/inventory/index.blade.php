...
<div class="panel">
    ...
    <div class="panel-heading bord-btm clearfix pad-all h-100">
        <h3 class="panel-title pull-left pad-no">{{ translate($type.' Inventory') }}</h3>
    ...
    <div class="panel-body">
        <table class="table table-striped res-table mar-no" cellspacing="0" width="100%">
            ...
            {{-- @foreach ($product->stocks->where('product_id', $product->id) as $key => $prd)--}} 
            <tr>
                     ...
                     <td><i class="fa fa-eye" onclick="stock_view(this)" value="{{ $product->id }}" ></i>
                            </td>
                     {{-- <td>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    @if ($type == 'Seller')
                                        <li><a href="{{route('seller.inventory.edit', encrypt($product->id))}}">{{translate('Edit')}}</a></li>
                                    @else
...
<div class="modal fade bd-example-modal-lg" id="stockModal" data-target="#update_reasonMoal">
<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <h4 class="modal-title"><strong>Stock</strong></h4>
    ...
    <script type="text/javascript">
        ...
        $('#status').change(function() {
            ...
            var selectedval = $('#hideval').val();
                $.post('{{ route('products.status') }}', {_token:'{{ csrf_token() }}', id:selectedrsn, status:selectedval, reason:selectedReason }, function(data){
                            if(data == 1){
            ...
            function stock_view(element){
            var value = element.getAttribute("value");
            $('#stockModal').modal('show');
            $.post('{{ route('inventory.stock') }}', {_token:'{{ csrf_token() }}', id:value }, function(data){
            
            var product_stock = data.product_stock;
            var products = data.products;
            
            if (products.length === 1 && product_stock.length > 0) {
                var product = products[0]; // Single product
                $('#stock_detail').empty(); // Clear the table body before appending new rows
                for (let index = 0; index < product_stock.length; index++) {
                    var stock = product_stock[index];
                    var name = product.name;
                    var variant = stock.variant;
                    var num_of_sale_variant = stock.num_of_sale_variant;
                    var qty = stock.qty;
                    var thumbnail_img = product.thumbnail_img
                    var url  = baseUrl+'/';
                    $('#stock_detail').append('<tr><td>' + (index + 1) + '</td><td><div class="media-left"><img loading="lazy"  class="img-md" src="'+url+thumbnail_img+'" alt="Image"></div><div class="media-body">'+name+'</div></td><td>' + variant + '</td><td>' + num_of_sale_variant + ' Times</td><td>' + qty + '</td></tr>');
                }
                } 
                // else {
                // $('#stock_detail').empty(); // Clear the table body if no data is available or arrays have different lengths
                // }
            });
        }
