    ...
    <div class="panel">
    	<div class="panel-body">
            ...
            <div class="row">
                ...
                {{-- @php
                    $delivery_boy = \DB::select('select users.*,delivery_boy_details.pincode from users INNER JOIN staff ON users.id = staff.user_id INNER JOIN delivery_boy_details ON users.id = delivery_boy_details.user_id where staff.role_id = 3 and users.del_ver = 1');
                @endphp 
               <div class="col-lg-3">
                    <label for=update_delivery_status"">{{translate('Assign Delivery Boy')}}</label>
                    <select class="form-control demo-select2"  data-minimum-results-for-search="Infinity" id="update_delivery_boy">
                      @if($order->assign_to == "")
                      <option value="">{{translate('Assign Delivery Boy')}}</option>
                      @endif
                      @foreach($delivery_boy as $db)
                        <option value="{{$db->id}}" @if ($order->assign_to == $db->id) selected @endif>{{$db->name}} {{$db->pincode}}</option>
                      @endforeach
    		...
    		<div class="row">
    			<div class="col-lg-12 table-responsive">
    				<table class="table table-bordered invoice-summary">
        				...
        				<tbody>
                            @php
                                $admin_user_id = \App\User::where('user_type', 'seller')->first()->id;
                            @endphp
