                ...
                @foreach ($orders as $key => $order_id)
                    ...
                    @if($order != null)
                        <tr>
                            ...
                            <td>
                            {{ count($order->orderDetails->whereIn('seller_id', $admin_user_id)) }}
                            </td>
                            <td>
                                @if ($order->user != null)
                                    {{ $order->user->name }}
                            ...
                            {{ single_price($order->orderDetails->whereIn('seller_id', array_merge([$order->seller_id], $admin_user_id))->sum('price') + $order->orderDetails->whereIn('seller_id', array_merge([$order->seller_id], $admin_user_id))->sum('tax')) }}
                            ...
                            <td>
                                ...
                                <span class="badge badge--2 mr-4">
                                    @if ($order->orderDetails->whereIn('seller_id', $seller_ids)->where('payment_status', 'paid')->isNotEmpty())   
